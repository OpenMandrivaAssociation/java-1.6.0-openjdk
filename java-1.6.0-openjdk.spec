# By default, we build only the development bits of 1.6
# because they're still needed (e.g. for building Android,
# dalvik/dex haven't been updated to work with 1.7's changes
# to the class file format etc).
# The JRE is end-of-lifed and not generally useful, since class
# and jar files generated by 1.6 are supposed to work fine with
# a 1.7 based runtime.
%bcond_with jre
%bcond_with src
...
%if %{with gcjbootstrap}
  %define icedteaopt		%{systemtapopt} --enable-bootstrap
%else
  %ifarch %{jit_arches}
...
Patch4:		java-1.6.0-openjdk-workarounds-to-allow-bootstrapping-with-gcj.patch
...
%else
# We still require a base package - but we can just use the newer 1.7
...
# Applied upstream as of icedtea 1.12.5
#patch3 -p1 -b .libXp~
...
%if %{with gcjbootstrap}
# Make sure we get the newly built libjli and not a system version (1.7's
# libjli is not ABI compatible with 1.6)
export LD_LIBRARY_PATH=`pwd`/openjdk.build-ecj/j2sdk-image/jre/lib/*/jli
%endif
...
%{configure2_5x}					\
	...
	--with-pkgversion=openmandriva-%{release}-%{_arch}	\
	--enable-pulse-java				\
...
%if %{with gcjbootstrap}
# Apply some temporary workarounds so we get an initial build with gcj
# going (afterwards, we revert those changes and rebuild relevant bits
# with OpenJDK)
%patch4 -p0 -b .gcjcompathacks~
%endif

%make STATIC_CXX=false

%if %{with gcjbootstrap}
# Build again with the just-built openjdk so we can get rid of the
# hacks needed to make it build with ecj...
mv openjdk.build bootstrap-openjdk
mv bootstrap-openjdk/j2re-image bootstrap-openjdk/jre
rm -rf bootstrap openjdk openjdk.build-ecj openjdk-ecj stamps *.build
%{configure2_5x}					\
	--disable-bootstrap --with-jdk-home=`pwd`/bootstrap-openjdk %{systemtapopt} \
	--with-openjdk-src-zip=%{SOURCE1}		\
	--with-pkgversion=openmandriva-%{release}-%{_arch}	\
	--enable-pulse-java				\
	--with-jaf-drop-zip=%{SOURCE9}			\
...
%if ! %{with jre}
# Remove the JRE -- we're just after the JDK
cat %name.files %{name}-demo.files |while read r; do
...
rm -f %buildroot%{_jvmdir}/%{jredir}/lib/fontconfig.properties
rm -rf %buildroot%_jvmdir/%sdklnk/{demo,sample}

# And replace JRE components of the JDK with symlinks into the 1.7 tree
...
%endif
%if ! %{with src}
rm -rf %buildroot%{_jvmdir}/%{sdkdir}/src.zip
%endif
